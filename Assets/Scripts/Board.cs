    ...
    private void SetUp() //fills the arrays
    {
        for (int i = 0; i < width; i++)
        {
            for (int j = 0; j < height; j++)
            {
                ...
                Vector2 tempPosition2 = new Vector2(i + 1, j);
                if (i >= 1)
                {
                    tempPosition = tempPosition2;
                }
                Vector2 tempPosition3 = new Vector2(i - 1, j);
                if (i >= 3)
                {
                    tempPosition = tempPosition3;
                }
                GameObject dot = Instantiate(dots[dotTouse], GetWorldPosition(i * xSpace, j * ySpace), Quaternion.identity);
